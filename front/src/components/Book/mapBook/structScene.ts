export const structScene = {
    ...
    'Image prompt': {
        ...
        requirements: '**Задача:** Выступи в роли эксперта по промптингу для AI-генераторов изображений (Nanobana/Stable Diffusion). На основе предоставленного художественного описания сцены, сгенерируй финальный, структурированный промпт, сфокусированный на четком позиционировании и композиции персонажей.\n' +
            ...
            '\n' +
            '1.  **Начало:** Начни с определения стиля и ракурса (например, "Full body shot, cinematic lighting, highly detailed concept art").\n' +
            ...
            '3.  **Структура персонажей:** Опиши каждого персонажа отдельно, используя четкое **позиционирование** и **скобки** для группировки информации. Используй ключевые слова, такие как **(left), (center), (right), (in the foreground), (behind)**.\n' +
            '4.  **Формат персонажей:** Описание каждого персонажа должно быть структурировано как:\n' +
            '    `Character N (позиция): [описание персонажа, его поза, действие, детали].`\n' +
